...
def _web_search(query: str, max_results: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code != 200:
            return {"error": f"Search failed: HTTP {response.status_code}", "results": []}
...
def _web_fetch(url: str, max_chars: int = 5000) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code != 200:
            return {"error": f"Fetch failed: HTTP {response.status_code}", "content": ""}
...
def _get_sec_filings(ticker: str, filing_type: str = "", max_results: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        response = requests.get(url, timeout=10, headers={
            'User-Agent': 'Mozilla/5.0 (research-agent; contact@example.com)'
        })
        
        if response.status_code != 200:
            return {"error": f"SEC lookup failed: HTTP {response.status_code}", "filings": []}
        ...
        for entry in entries[:max_results]:
            ...
            if title_match and link_match:
                ...
                filing_type_extracted = title.split(' - ')[0] if ' - ' in title else title[:20]
...
def _get_analyst_estimates(ticker: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if result["price_targets"]["mean"] and result["price_targets"]["current_price"]:
            ...
            result["price_targets"]["upside_to_mean"] = round((target - current) / current * 100, 1)
...
def execute_tool(tool_name: str, tool_input: Dict[str, Any]) -> str:
    ...
    executors = {
        "web_search": lambda inp: _web_search(
            inp.get("query", ""),
            inp.get("max_results", 5)
        ),
        "web_fetch": lambda inp: _web_fetch(
            inp.get("url", ""),
            inp.get("max_chars", 5000)
        ),
        "get_price_data": lambda inp: _get_price_data(
            inp.get("ticker", "")
        ),
        ...
        "get_company_info": lambda inp: _get_company_info(
            inp.get("ticker", "")
        ),
...
if __name__ == "__main__":
    ...
    result = execute_tool("web_search", {"query": f"{ticker} earnings 2026", "max_results": 3})
    print(result[:500] + "...\n")
