...
async def get_stock_detail(ticker: str):
    ...
    try:
        with get_db() as conn:
            ...
            LIMIT 1
            """
            fundamentals_result = conn.execute(
                fundamentals_sql, {"ticker": ticker}
            ).fetchdf()

            fundamentals_data = {}
            if len(fundamentals_result) > 0:
                fundamentals_data = clean_dict(
                    fundamentals_result.to_dict("records")[0]
                )
            ...
            LIMIT 1
            """
            technicals_result = conn.execute(
                technicals_sql, {"ticker": ticker}
            ).fetchdf()
        ...
        logger.exception("Stock detail lookup failed")
