...
AGENT_METHODOLOGY_MAP = {
    "08-DATA-QUALITY-AGENT": {
        ...
        "role": "data_requirements",  # Focus on what data structures are needed
        "instructions": """
...
""",
    ...
    "02-QUANT-AGENT": {
        ...
        "role": "methodology_application",  # Focus on applying methods correctly
        "instructions": """
...
""",
    ...
    "04-COMPETITIVE-AGENT": {
        "concepts": [],
        "role": None,
        "instructions": None,  # No AFML context needed
    },
...
class MethodologyInjector:
    ...
    Usage:
        ...
        # Get just the data requirements (for Data Quality Agent)
        data_reqs = injector.get_data_requirements("triple_barrier")
    """
    ...
    def get_context(self, agent_name: str) -> Optional[str]:
        """
        Get methodology context for a specific agent.
        
        Returns None if agent doesn't need methodology context.
        """
    ...
    def get_relevant_concepts(self, agent_name: str) -> list[MethodologyConcept]:
        """Get list of methodology concepts relevant to an agent."""
        if agent_name not in self.agent_map:
            return []
    ...
    def format_for_prompt(self, agent_name: str) -> str:
        """
        Format methodology context ready for injection into prompt.
        
        Returns empty string if no context needed.
        """
...
def integrate_with_orchestrator_example():
    """
    Example of how to integrate with orchestrator.py
    
    In orchestrator.py, modify _build_user_message():
    
    ```python
    from sandbox.methodology_injection import MethodologyInjector
    
    class AgentExecutor:
        def __init__(self, ...):
            ...
            self.methodology = MethodologyInjector()
        
        def _build_user_message(self, agent_name, context, additional_context):
            # Get methodology context for this agent
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("ALL DATA REQUIREMENTS:")
    print("=" * 60)
    
    for concept, reqs in injector.get_all_data_requirements().items():
        print(f"\n{concept}:")
        for req in reqs:
            print(f"  - {req}")
