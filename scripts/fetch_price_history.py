...
def fetch_prices(ticker: str, years: int = 2, conn=None):
    ...
    logger.info(
        f"Fetching {ticker} prices from {start_date.date()} to {end_date.date()}..."
    )

    try:
        # Download data. `timeout=30` so a stalled Yahoo connection
        # doesn't hang the whole fetch loop forever — without it,
        # yfinance defaults to the underlying requests library's
        # behaviour (no timeout = block indefinitely).
        df = yf.download(
            ticker, start=start_date, end=end_date, progress=False, timeout=30
        )
        ...
        if conn:
            conn.execute(
                "DELETE FROM prices WHERE ticker = $ticker", {"ticker": ticker}
            )
...
def main():
    ...
    parser.add_argument(
        "--all-stocks", action="store_true", help="Fetch for all stocks in database"
    )
    parser.add_argument(
        "--years", type=int, default=2, help="Number of years of history (default: 2)"
    )
    ...
    logger.info(
        f"Fetching {args.years} years of price data for {len(tickers)} stocks..."
    )
